...
document.querySelector("#btn1").addEventListener("click", () => {
  ...
  list[3].parentNode.style.backgroundColor = "skyblue"; //ul
  ...
  // list[9].style.backgroundColor = "pink"; 이것도 가능
  ...
  list[11].append("12345","abcd","가나다라"); // 위의 appendChild와 같다.

  // 3. 첫 번째 자식으로 추가 (요소.prepend("내용" 또는 노드 또는 요소 n개))
  list[11].prepend("오늘", "점심", "뭐 먹냐");

  // 4. 이전/다음 형제로 추가 (요소.before / 요소.after("내용" 또는 노드 또는 요소 n개))
  // #test 전/후에 내용추가
  test.before("이전입니다", "뇸뇸");
  test.after("이후입니다");
...
document.querySelector("#btn2").addEventListener("click", () => {
  ...
  console.log(test2.children); //HTMLCollection으로 넘어옴
  ...
  // #test2의 부모 요소
  console.log(test2.parentElement); //ul의 부모 요소는 body
